	...
	case WM_NOTIFY:
		...
		case NM_RETURN: {
			...
			if (link->hdr.idFrom == IDC_SYSLINK_HOME_PAGE)
				ShellExecute(NULL, _T("open"), _T("https://github.com/teppyboy/OpenKey"), NULL, NULL, SW_SHOWNORMAL);
			else if (link->hdr.idFrom == IDC_SYSLINK_NEW_VERSION)
				ShellExecute(NULL, _T("open"), _T("https://github.com/teppyboy/OpenKey/releases"), NULL, NULL, SW_SHOWNORMAL);
