...
void OpenKeyInit() {
	...
	APP_GET_DATA(vHideWindowsVietnameseIME, 0);
	...
	DEBUG_LOG(L"OpenKey config: lang=%d input=%d code=%d spell=%d modern=%d restore=%d macro=%d macroEng=%d smart=%d remember=%d tsf=%d forceUnload=%d tsfCompatBackspace=%d hideWinViIme=%d gray=%d metro=%d chromium=%d switch=0x%08X convertHotkey=0x%08X",
		...
		vRememberCode, vUseTSFInput, vForceUnloadTSFDLL, vTSFBackspaceCompatibility, vHideWindowsVietnameseIME, vUseGrayIcon, vSupportMetroApp, vFixChromiumBrowser,
